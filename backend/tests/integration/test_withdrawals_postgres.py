...
def test_withdrawal_reserves_available_funds_and_reconciliation_sees_hold(db, monkeypatch) -> None:
    monkeypatch.setattr(WithdrawalService, "calculate_fee", classmethod(lambda cls, db, amount: Decimal("0.00")))
    user = make_user(
        db,
...
def test_cancelling_withdrawal_releases_hold_and_reverses_original_transaction(db, monkeypatch) -> None:
    monkeypatch.setattr(WithdrawalService, "calculate_fee", classmethod(lambda cls, db, amount: Decimal("0.00")))
    user = make_user(
        db,
    ...
    withdrawal = WithdrawalService.cancel_by_user(db, user=user, withdrawal_id=withdrawal.id)
    db.commit()
    db.expire_all()
    ...
    assert ReconciliationService.account(db, account)["held_matches"] is True


def test_completed_withdrawal_consumes_hold_without_refunding_available_balance(db, monkeypatch) -> None:
    monkeypatch.setattr(WithdrawalService, "calculate_fee", classmethod(lambda cls, db, amount: Decimal("0.00")))
    user = make_user(
        db,
    ...
    WithdrawalService.approve(db, admin=admin, withdrawal_id=withdrawal.id)
    db.commit()
